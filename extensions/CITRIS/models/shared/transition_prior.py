...
class TransitionPrior(nn.Module):
    ...
    def __init__(self, num_latents, num_blocks, c_hid,
        ...
        history_length : int
                             Number of previous time steps to use in the model
        """
        super().__init__()
        self.num_latents = num_latents
        self.imperfect_interventions = imperfect_interventions
    ...
    def _get_prior_params(self, history, target=None, target_prod=None, target_samples=None, z_t1=None):
        ...
        Parameters
        ----------
        history : torch.FloatTensor, shape [batch_size, history_length*num_latents]
        ...
        if self.autoregressive_model:
            ...
            if history.shape[-1] != self.num_latents:
    ...
    def kl_divergence(self, history, target, z_t1_mean, z_t1_logstd, z_t1_sample):
        """
        ...
        Parameters
        ----------
        history : torch.FloatTensor, shape [batch_size, num_samples, num_latents]
              Latents at time step t, i.e. the input to the prior
        target : torch.FloatTensor, shape [batch_size, num_blocks]
                 The intervention targets I^t+1
        ...
        if self.autoregressive_model:
            ...
            prior_params = self._get_prior_params(history, target_samples=full_target_samples, target=target_exp, z_t1=z_t1_sample)
            ...
            net_inps = history[:,None].expand(-1, self.num_blocks+1, -1).flatten(0, 1)
            target_samples = torch.eye(self.num_blocks+1, device=history.device)[None]
            ...
            target_prod = target_prod[:,:,None].expand(-1, -1, history.shape[-1], -1)
    ...
    def sample_based_nll(self, z_t, z_t1, target):
        ...
        target : torch.FloatTensor, shape [batch_size, num_blocks]
                 The intervention targets I^t+1
        """
        batch_size, num_samples, history_dim = z_t.shape
        latent_dim = self.num_latents
        ...
        if self.autoregressive_model:
            ...
            prior_mean, prior_logstd = [p.unflatten(0, (batch_size, num_samples)) for p in prior_params]
            assert not torch.isnan(z_t).any(), "NaN detected in latent history"
            assert not torch.isnan(z_t1).any(), "NaN detected in next latent state"

            assert not torch.isnan(prior_mean).any(), "NaN detected in transition prior mean"
            assert not torch.isnan(prior_logstd).any(), "NaN detected in transition prior log std"
            ...
            intv_params = self._get_intv_params((batch_size, num_samples, self.num_latents), target=None)
            intv_mean, intv_logstd = intv_params[0], intv_params[1]
            nll_intv = -gaussian_log_prob(
                intv_mean[:,None,:,:],
                intv_logstd[:,None,:,:],
                z_t1[:,:, :, None]
            )
