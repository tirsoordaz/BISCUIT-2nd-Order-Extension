...
class CITRISVAE(pl.LightningModule):
    ...
    def _get_loss(self, batch, mode='train'):
        ...
        if self.hparams.use_flow_prior:
            ...
            kld_t1_all = self.prior_t1.kl_divergence(history=history,
                                                    target=target.flatten(0,1),
        ...
        # Logging
        self.log(f'{mode}_kld_t1', kld_t1_all.mean() / (imgs.shape[1]-history_length))
        ...
        assert not torch.isnan(out_nll).any(), "NaN detected in NLL loss"
        assert not torch.isnan(kld).any(), "NaN detected in KL divergence"
        assert not torch.isnan(rec_loss).any(), "NaN detected in reconstruction loss"
